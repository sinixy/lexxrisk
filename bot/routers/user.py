...
async def current_risk(message: types.Message, user: User):
    risk = await manager.get_current_risk(user.account_id)
    await message.answer(f"Ваши условия:\n\nСтоп: {risk.stop}\nКавер: {risk.cover}")

@router.message(F.text == "Изменить стоп")
async def change_stop(message: types.Message, user: User, state: FSMContext):
    risk = await manager.get_current_risk(user.account_id)
...
async def change_cover(message: types.Message, user: User, state: FSMContext):
    risk = await manager.get_current_risk(user.account_id)
...
async def change_fixer(message: types.Message, user: User, state: FSMContext):
    fixer = await manager.get_current_fixer(user.account_id)
...
async def change_risk_val(message: types.Message, user: User, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("Некорректное значение! Попробуйте ещё раз:", reply_markup=cancel_kb)
        return
...
async def confirm_risk_change(callback: types.CallbackQuery, user: User, state: FSMContext):
    data = await state.get_data()
